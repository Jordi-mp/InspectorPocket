...
""" This module provides a class to handle the production of the grid and all related operations. """

class GridError(Exception):
  """ Exception for Grid class in case there are gird errors."""
...
class Grid:
  def __init__(self):
    """ A class to handle the production of the grid and all related operations."""
  ...
  def initialize_grid_coords(self, coordinates, margin=None, gridspacing=None):
    """ Initializes grid with a list of coordinates."""
  ...
  def initialize_grid_center(self, center, width, margin=None, gridspacing=None):
    """ Initializes grid with center point and width of grid along coordinate axis."""
    self.initialized = False
  ...
  def project_atoms(self, molecule, altCoordinates=None):
      """ Project the atoms onto the grid (previously reset)."""
  ...
  def cell_indexing(self):
    """ Assigns indexes the cells to access the grid points."""
    if not self.initialized: raise GridError("Grid not initialized.")
  ...
  def detect_buried_cells(self, minLength=10, dobThreshold=9, minNeighbours=9):
    """ Gets the indices of empty cells that have more than dobThreshold non-empty neighbours.
        Check that each of these cells has at least minNeighbours neighbours."""
    for x,y,z in self.cell_indexing(): self.cells_buried[x][y][z] = False
  ...
  def empty_cell_indexing(self):
    """ Lists the indices to access empty grid points."""
    if not self.initialized: raise GridError("Grid not initialized.")
    for ix,iy,iz in self.cell_indexing():
      if not self.cells_occupied[ix][iy][iz]: 
        yield (ix,iy,iz)

  def list_neighbors(self, ind_of_cell, ind_grid_vector):
    """ Get list of neighbours for an index. """
    neighs = []
    indx = ind_of_cell[0]
    indy = ind_of_cell[1]
    indz = ind_of_cell[2]
  ...
  def ind_grid_vector(self, ind_cells):
    """ Maps a list of indices onto a vector that contains -1 or the index position for indices
        in the list. """
  ...
  def cluster_buired_cells(self):
    """ Clusters the buried cells and returns the clusters."""
    if not self.initialized: raise GridError("Grid not initialized.")
    if not self.atomsAssigned: raise GridError("No atoms assigned to Grid")
