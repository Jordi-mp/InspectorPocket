...
""" InspectorPocket is a standalone open source software that identifies and visualizes pockets in protein structures,
as well as the residues in the vicinity of each pocket."""
...
class InspectorPocket:
    """ Class that encapsulates the identification and visualization of pockets in protein structures."""
    def __init__(self, options):
        ...
        self.fileops = FileOps(pdbfile=self.pdb, tempDir=self.out_directory)
        self.iterpockets = IteratePockets(pdbfile=self.pdb,
    ...
    def modify_site_numbers(self):
        """ Modify the site numbers in the PDB files."""
    ...
    def generate_bash_script(self):
        """ Generate a bash script to concatenate individual pocket PDB files into one."""
        bash_script = f'''
#!/bin/bash
touch {self.pdb}_pockets.pdb
cat *.pdb >> {self.pdb}_pockets.pdb
grep '^ATOM' {self.pdb}_pockets.pdb > {self.pdb}_pockets_filtered.pdb
mv {self.pdb}_pockets_filtered.pdb {self.pdb}_all_pockets.pdb
'''
        return bash_script

    def execute_bash_script(self):
        """ Execute the bash script to concatenate individual pocket PDB files into one."""
        output_directory = f"{self.pdb[:-4]}"  
        if not os.path.exists(output_directory):
            print(f"Output directory '{output_directory}' does not exist.")
            return None 
    ...
    def generate_pymol_script(self, output_script_file):
        """ Generate a PyMOL script to visualize the pockets in the protein structure."""
        output_directory = f"{self.pdb[:-4]}"
        if not os.path.exists(output_directory):
            print(f"Output directory '{output_directory}' does not exist.")
            return None
...
"""
    ...
    def generate_chimera_script(self, output_script_file):
        """ Generate a Chimera script to visualize the pockets in the protein structure."""
        output_directory = f"{self.pdb[:-4]}"
        if not os.path.exists(output_directory):
            print(f"Output directory '{output_directory}' does not exist.")
            return None
...
"""
    ...
    def process_pockets_and_residues(self):
        ...
        try:
            ...
            print("Report generated successfully. Thank you for using InspectorPocket!")
