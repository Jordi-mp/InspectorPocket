...
def calculate_connectivity(voronoi):
    """Calculate the connectivity of Voronoi cells."""
    connectivity = []
    for i in range(len(voronoi.ridge_vertices)):
        ridge_vertices = voronoi.ridge_vertices[i]
        if -1 not in ridge_vertices:  # Check if the ridge is finite
            neighbors = []
            for j in ridge_vertices:
                if j < len(voronoi.regions):  # Check if the index is within range
                    neighbors.extend(voronoi.regions[j])
            neighbors = set(neighbors)
            neighbors.discard(-1)  # Remove -1 if present
            neighbors.discard(i)   # Remove current cell index
            connectivity.append(len(neighbors))
        else:
            connectivity.append(0)  # Set connectivity to 0 for infinite ridges
    return connectivity
...
def extract_features(voronoi, proximity_scores, atomic_neighbors, vertex_neighbors, cell_volumes):
    ...
    features = []
    connectivity = calculate_connectivity(voronoi)
    for i, cell in enumerate(voronoi.regions):
        ...
        feature_vector = [proximity_score, atomic_neighbor_count, vertex_neighbor_count, volume, conn]
...
def extract_features_non_binding(voronoi_non_binding, proximity_scores_non_binding, atomic_neighbors_non_binding, vertex_neighbors_non_binding, cell_volumes_non_binding):
    ...
    connectivity = calculate_connectivity(voronoi_non_binding)
    for i, cell in enumerate(voronoi_non_binding.regions):
        ...
        # Build feature vector
        feature_vector_non_binding = [proximity_score_non_binding, atomic_neighbor_count_non_binding, vertex_neighbor_count_non_binding, volume_non_binding, conn]
